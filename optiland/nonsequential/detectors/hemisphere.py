...
class HemisphereDetector(FarFieldDetector):
    ...
    def _cos_theta(self, lz):
        ...
        Args:
            lz: Local z direction component per ray, shape (N,), masked to
                zero for rays that did not hit.

        Returns:
            ``cos(theta)`` in [0, 1], shape (N,). The shell only accepts
            rays crossing it outward, so the clamp at 0 is a guard on
            rounding at the equator, not a fold of one hemisphere onto the
            other.
        """
        return be.clip(lz, 0.0, 1.0)
